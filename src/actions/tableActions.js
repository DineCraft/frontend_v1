...
export const allTables = () => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:3005/api/tables/all`, config);
...
export const listTables = (keyword = "", pageNumber = "") => async (
    ...
    try {
        ...
        const { data } = await axios.get(
            `http://localhost:3005/api/tables?keyword=${keyword}&pageNumber=${pageNumber}`,
            config
        );
...
export const createTable = (table) => async (dispatch, getState) => {
    ...
    try {
        ...
        //create table
        const { data } = await axios.post("http://localhost:3005/api/tables", { name }, config);
        dispatch({
...
export const listTableDetails = (id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:3005/api/tables/${id}`, config);
        dispatch({
...
export const updateTable = (table) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.put(
            `http://localhost:3005/api/tables/${table.id}`,
            table,
            config
        );
        dispatch({
...
export const deleteTable = (id) => async (dispatch, getState) => {
    try {
        ...
        await axios.delete(`http://localhost:3005/api/tables/${id}`, config);
        dispatch({
