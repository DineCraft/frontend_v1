    ...
    async (dispatch, getState) => {
        try {
            ...
            const { data } = await axios.get(
                `http://localhost:3005/api/clients?keyword=${keyword}&pageNumber=${pageNumber}`,
                config
            );
...
export const createClient = (client) => async (dispatch, getState) => {
    try {
        ...
        //create client
        const { data } = await axios.post("http://localhost:3005/api/clients", client, config);
        dispatch({
...
export const listClientDetails = (id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:3005/api/clients/${id}`, config);
        dispatch({
...
export const updateClient = (client) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.put(
            `http://localhost:3005/api/clients/${client.id}`,
            client,
            config
        );
        dispatch({
...
export const deleteClient = (id) => async (dispatch, getState) => {
    try {
        ...
        await axios.delete(`http://localhost:3005/api/clients/${id}`, config);
        dispatch({
